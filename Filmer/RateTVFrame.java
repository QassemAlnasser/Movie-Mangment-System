...
public class RateTVFrame extends JFrame {
  ...
  public RateTVFrame() {
       ...
       Rate.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
           int r = rateSilder.getValue();
            RegQueries q = new RegQueries ();
                 String s = q.AddRating_TV(TV_Name,(int) r, UserName);
                 if(s=="Error") { result.setText("Rating Exists\nPress Update to update it"); Rate.setVisible(false); Update.setVisible(true);  }
                 else {System.out.println(TV_Name+" is ratted secussefully"); setVisible(false);  
                        result.setText("");
        ...
        public void actionPerformed(ActionEvent e) {
           int r = rateSilder.getValue();
            RegQueries q = new RegQueries ();
                 q.UpdateRating_TV(TV_Name, r, UserName);  
                 setVisible(false);
                     result.setText("");
                          rateSilder.setValue(5);
                          Update.setVisible(false);
